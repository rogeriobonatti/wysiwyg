...
def gen_ann(dir_names, root_dir):
    ...
    for dir_name in dir_names:
        ...
        base_numpy = os.path.join(dir_name, 'processed_withpose3')
        ...
        for i, numpy_file in enumerate(numpy_list):
            ...
            language_bias = data['language_bias']
            for j in range(num_images):
                ...
                img_bev_rel_path = os.path.join(dir_name, 'processed_images_bev_fixed7', str(i), str(j)+'.png')
                ...
                frame_ann = {
                    ...
                    'img_number': j,
                    'goal': goal.tolist(),
                    'speed': language_bias[0,1],
                    'direction_bias': language_bias[0,2],
...
root_dir = '/home/azureuser/hackathon_data/hackathon_data_language'
...
with open(os.path.join(root_dir, 'train_ann_pose_lang_debug.json'), 'w') as f:
    json.dump(ann, f, indent=4)

# Full training set.
ann = gen_ann(dir_names[2:], root_dir)
with open(os.path.join(root_dir, 'train_ann_pose_lang.json'), 'w') as f:
    json.dump(ann, f, indent=4)
