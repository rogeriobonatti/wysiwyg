...
def compute_bev_image(original_points, sensor_origins, time_stamps, pc_range, voxel_size):
    ...
    vis_mat = -1.0* np.ones(shape=(nx, ny))
    original_points_idx = np.floor(original_points / voxel_size).astype(int) # becomes indexes instead of meters
    # remove indexes out of bounds for the image
    filtered_points_idx = original_points_idx[(original_points_idx[:,0]>0) & \
                                              (original_points_idx[:,0]<=(nx-1)) & \
                                              (original_points_idx[:,1]>0) & \
                                              (original_points_idx[:,1]<=(ny-1))] 
...
def process_folder(folder, processed_dataset_path):
    ...
    if parallel_processing is True:
        ...
        for i, episode_name in enumerate(episodes_list, 0):
            episode_folder = os.path.join(folder, 'processed_images_bev/', str(i))
...
output_folder_name = 'processed_images_bev'
